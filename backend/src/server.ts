...
federation.setActorDispatcher('/users/{identifier}', FederationHandlers.handleActorRequest).setKeyPairsDispatcher(FederationHandlers.handleKeyPairsRequest);

federation.setOutboxDispatcher('/users/{identifier}/outbox', FederationHandlers.handleOutboxRequest);

federation.setObjectDispatcher(Note, '/users/{identifier}/notes/{noteId}', FederationHandlers.handleNoteRequest);
...
federation.setFollowingDispatcher('/users/{identifier}/following', FederationHandlers.handleFollowingRequest);
...
serve({
    fetch: async (request: Request) => {
        ...
        // Add CORS and ActivityPub headers for all requests
        const corsHeaders = {
        ...
        // Add ActivityPub headers if needed
        ...
        if (url.pathname === '/auth/complete-profile' && request.method === 'POST') {
            const response = await requireAuth(AuthHandlers.handleCompleteProfile)(request);
            const responseHeaders = new Headers(response.headers);
            Object.entries(corsHeaders).forEach(([key, value]) => {
                responseHeaders.set(key, value);
            });
        ...
        if (url.pathname === '/auth/profile' && request.method === 'GET') {
            const response = await requireAuth(AuthHandlers.handleGetProfile)(request);
            const responseHeaders = new Headers(response.headers);
            Object.entries(corsHeaders).forEach(([key, value]) => {
                responseHeaders.set(key, value);
            });
        ...
        if (url.pathname === '/auth/profile' && request.method === 'PUT') {
            const response = await requireAuth(AuthHandlers.handleUpdateProfile)(request);
            const responseHeaders = new Headers(response.headers);
            Object.entries(corsHeaders).forEach(([key, value]) => {
                responseHeaders.set(key, value);
            });
        ...
        if (url.pathname === '/auth/user/posts' && request.method === 'GET') {
            const response = await requireAuth(AuthHandlers.handleGetUserPosts)(request);
            const responseHeaders = new Headers(response.headers);
            Object.entries(corsHeaders).forEach(([key, value]) => {
                responseHeaders.set(key, value);
            });
        ...
        if (url.pathname === '/auth/user/followers' && request.method === 'GET') {
            const response = await requireAuth(AuthHandlers.handleGetFollowers)(request);
            const responseHeaders = new Headers(response.headers);
            Object.entries(corsHeaders).forEach(([key, value]) => {
                responseHeaders.set(key, value);
            });
        ...
        if (url.pathname === '/auth/user/following' && request.method === 'GET') {
            const response = await requireAuth(AuthHandlers.handleGetFollowing)(request);
            const responseHeaders = new Headers(response.headers);
            Object.entries(corsHeaders).forEach(([key, value]) => {
                responseHeaders.set(key, value);
            });
        ...
        if (url.pathname === '/auth/user/logged-in' && request.method === 'GET') {
            const response = await requireAuth(AuthHandlers.handleGetLoggedInUser)(request);
            const responseHeaders = new Headers(response.headers);
            Object.entries(corsHeaders).forEach(([key, value]) => {
                responseHeaders.set(key, value);
            });
        ...
        if (likeParts.length === 5 && likeParts[1] === 'api' && likeParts[2] === 'posts' && likeParts[4] === 'like') {
        ...
        if (url.pathname === '/api/users/discover' && request.method === 'POST') {
            try {
                ...
                if (!handle) {
                    return new Response(JSON.stringify({ error: 'Missing required field: handle' }), {
                        status: 400,
                        headers: { 'Content-Type': 'application/json' }
                    });
                }
                ...
                if (handleParts.length !== 2) {
                    return new Response(JSON.stringify({ error: 'Invalid handle format' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
                }
        ...
        if (url.pathname === '/api/users/search' && request.method === 'GET') {
            try {
                ...
                if (limit < 1 || limit > 50) {
                    return new Response(JSON.stringify({ error: 'Limit must be between 1 and 50' }), {
                        status: 400,
                        headers: { 'Content-Type': 'application/json' }
                    });
                }
