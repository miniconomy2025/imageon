...
export class ActivityPubService {
    ...
    async saveActivity(activityId: string, activityType: string, actorId: string, objectId: string, additionalData?: Record<string, any>) {
        ...
        // Generate a proper activity ID using our URL structure
        const activityUuid = activityId.split('/').pop() || crypto.randomUUID();
        const properActivityId = `${config.federation.protocol}://${config.federation.domain}/users/${identifier}/activities/${activityUuid}`;

        const item: Record<string, any> = {
            PK: `ACTIVITY#${activityUuid}`,
