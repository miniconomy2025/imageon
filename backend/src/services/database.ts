...
export class DatabaseService {
    ...
    async queryItems(
        ...
        try {
            const command = new QueryCommand({
                TableName: this.tableName,
                KeyConditionExpression: options?.sortKeyExpression ? `PK = :pk AND ${options.sortKeyExpression}` : 'PK = :pk',
    ...
    async queryItemsByGSI1(
        ...
        try {
            const command = new QueryCommand({
                TableName: this.tableName,
                IndexName: 'GSI1',
                KeyConditionExpression: options?.sortKeyExpression ? `GSI1PK = :gsi1pk AND ${options.sortKeyExpression}` : 'GSI1PK = :gsi1pk',
                ExpressionAttributeValues: {
                    ':gsi1pk': gsi1pk,
                    ...options?.attributeValues
                },
    ...
    async queryItemsByGSI2(
        ...
        try {
            const command = new QueryCommand({
                TableName: this.tableName,
                IndexName: 'GSI2',
                KeyConditionExpression: options?.sortKeyExpression ? `GSI2PK = :gsi2pk AND ${options.sortKeyExpression}` : 'GSI2PK = :gsi2pk',
                ExpressionAttributeValues: {
                    ':gsi2pk': gsi2pk,
                    ...options?.attributeValues
                },
    ...
    /**
     * Get likes for a specific object (post/note)
     */
    ...
    /**
     * Get likes count for a specific object (post/note)
     */
    async getLikesCountForObject(objectId: string) {
        try {
            const command = new QueryCommand({
                TableName: this.tableName,
                IndexName: 'GSI2',
            ...
            console.error(`Error getting likes count for object ${objectId}:`, error);
            return 0;
        }
    }

    /**
     * Get all likes activities (for debugging/analytics)
     */
    async getAllLikes() {
        try {
            const command = new QueryCommand({
