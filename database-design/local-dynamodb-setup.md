...
const dynamoConfig = {
  region: process.env.DYNAMODB_REGION || "af-south-1",
...
const createLocalTables = async () => {
  ...
  try {
    ...
    try {
      await dynamoDB
        .describeTable({ TableName: "SocialMediaApp-Local" })
        .promise();
    ...
    await dynamoDB
      .waitFor("tableExists", { TableName: "SocialMediaApp-Local" })
      .promise();
  ...
  const samplePosts = [
    {
      ...
      content:
        "Just launched my new social media app! 🚀 Excited to share it with everyone.",
      content_type: "text",
...
describe("User API", () => {
  ...
  test("should get user profile", async () => {
    const response = await request(app).get("/api/users/john_doe").expect(200);
